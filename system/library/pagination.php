...
class Pagination {
	...
	public function render() {
		...
		if ($page > 1) {
			...
			if ($page == 2){
				$output .= '<a class="prev" href="' . str_replace('&', '&amp;', rtrim( str_replace('page={page}', '', $tmp_url), '?&')) . '"></a> ';
			}else{
				$output .= '<a class="prev" href="' . str_replace('{page}', $page - 1, $this->url) . '"></a> ';
			}
		} else {
			$output .= '<span class="prev"></span> ';
		}
   	...
   	if ($page < $num_pages) {
			$output .= ' <a class="next" href="' . str_replace('{page}', $page + 1, $this->url) . '"></a>';
		} else {
